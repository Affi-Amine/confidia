...
class IdentityWebPython(object):
    ...
    def _parse_redirect_errors(self, req_params: dict) -> None:
        ...
        self._logger.debug(f"Received redirect parameters: {req_params}")
    ...
    if str(AADErrorResponse.ERROR_CODE_PARAM_KEY) in req_params:
        ...
        self._logger.error(f"Error code received: {error_code}")
        ...
        if error_code.startswith(str(AADErrorResponse.B2C_FORGOT_PASSWORD_ERROR_CODE)):
            ...
            raise OtherAuthError(f"Unknown error while parsing redirect: {error_code}")
    else:
        self._logger.error("No error code found in redirect parameters")
