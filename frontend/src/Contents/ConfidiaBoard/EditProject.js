...
export default function EditProject() {
  ...
  const [selectedUser, setSelectedUser] = useState(
    mockProject.availableUsers[0].id
  );
    ...
    <>
      ...
      <form className="Form" onSubmit={(e) => e.preventDefault()}>
        ...
        <div>
          ...
          >
            <FontAwesomeIcon className="icon" icon="fa-solid fa-plus" /> Add
            User
          </button>
        </div>
        <label>{t("Projects.selectConnector")}:</label>
        <select
          value={data.connector.id}
          onChange={(e) => {
        ...
        <input type="submit" value={t("Projects.submit")} onClick={() => console.log(data)} />
      </form>
    </>
  );
}
